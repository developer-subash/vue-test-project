...
import WithRender from '@/Pages/Admin/Modules/ManagePackage/tab/UpcommingDeparture/Views/upcomming-departure.html';
...
export default class ManageDeparture extends Vue {
  ...
  public mounted() {
    ...
    if (this.$route.query.id) {
      ...
      this.Id = this.Departure.Trek.id;
    }
  }
  ...
  public EditDeparture(index: any, departure: any) {
    // console.log(" for Edit", departure.id);
    this.$router.push({
    ...
    // // console.log(departure.trek.title);
    // this.EditSelectedDeparture.Price = departure.price;
    // this.EditSelectedDeparture.Title = departure.trek.title;
  ...
  protected IsValidationTrueEdit() {
    if (!this.EditSelectedDeparture.Price) {
        Object.assign(this.errors, { edit_price: ['The price field is required.'] });
    }
  ...
  protected CreateDeparture() {
    ...
    let data = {
      ...
      price : this.Departure.Price ? this.Departure.Price : '',
    };
    // console.log(payLoad);
    this.ui.PostLoading = true;
    trekService
